...
def mean_objective(C_a_params, S_list, F_list, n):
    M = (len(C_a_params) - n * (n + 1) // 2) - 1
    a = C_a_params[:M + 1]
    C_params = C_a_params[M + 1:]
    ...
    for S, F in zip(S_list, F_list):
        ...
        total_loss += objective(F, F_pred)
    return total_loss / len(S_list)

print("Fitting a_m and C for the training set...")
start_time = time.time()
n = laplacian_matrices_training[0].shape[0]
lower_tri_indices = np.tril_indices(n)
num_params_C = len(lower_tri_indices[0])
M = 4
initial_a = np.random.uniform(low=0.01, high=0.1, size=M + 1)
initial_C_params = np.random.uniform(low=-0.01, high=0.01, size=num_params_C)
initial_params = np.concatenate([initial_a, initial_C_params])

result = minimize(
    mean_objective,
    initial_params,
    args=(structural_matrices_training, functional_matrices_training, n),
    method='L-BFGS-B',
    options={
        'maxiter': 500,
        'ftol': 1e-3
...
a_optimized = optimized_params[:M + 1]
C_params_optimized = optimized_params[M + 1:]
C_optimized = np.zeros((n, n))
C_optimized[lower_tri_indices] = C_params_optimized
C_optimized = C_optimized + C_optimized.T - np.diag(np.diag(C_optimized))
...
start_time = time.time()
for i in range(n_testing):
    S_powers_test = precompute_S_powers(structural_matrices_testing[i], a_optimized)
    F_pred = liang_model(S_powers_test, C_optimized)
...
plt.plot(range(M + 1), a_optimized, marker='o')
